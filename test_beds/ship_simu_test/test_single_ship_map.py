...
if test1:
    ...
    if plot_1:    
        ...
        # === Draw your simulation outputs ON THE SAME AXES ===
        ax.plot(own_ship_results_df['east position [m]'], own_ship_results_df['north position [m]'],
                color='#20334a', linewidth=1.6, label='Own ship track', zorder=5)
        ax.scatter(own_ship.auto_pilot.navigate.east, own_ship.auto_pilot.navigate.north,
                marker='x', s=28, color='blue', label='Waypoints', zorder=6)
        for x, y in zip(own_ship.ship_drawings[1], own_ship.ship_drawings[0]):
            ax.plot(x, y, color='blue', linewidth=0.9, zorder=7)
